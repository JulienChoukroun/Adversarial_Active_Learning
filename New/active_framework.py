...
def active_training(labelled_data, network_name, img_size,
    ...
    for i in range(repeat):
        ...
        earlyStopping=tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, verbose=0, mode='auto')
...
def egl_selection(model, unlabelled_data, nb_data):
    ...
    def get_gradient(model):
        ...
        y_pred = model.call(x)
        loss = K.mean(tensorflow.keras.losses.categorical_crossentropy(y, y_pred))

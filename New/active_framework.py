...
def active_training(labelled_data, network_name, img_size,
    ...
    for i in range(repeat):
        ...
        ##### keras.callback does not work with Tensorflow 2 sp I change it by tf.keras.callback
        earlyStopping=tf.keras.callbacks.EarlyStopping(monitor='loss', patience=10, verbose=0, mode='auto')
...
def get_weights(model):
    ...
    for layer in layers:
        if layer.trainable_weights:
            ...
            ##### The elem.get_value() function does not work in Tensorflow 2 so I change it by K.get_value(elem) function
    ...
    for layer in layers:
        if layer.trainable_weights:
            ...
            for elem in weights_layer:
                ##### The elem.set_value() function does not work with Tensorflow 2 so I change it by K.set_value(elem) function
...
def egl_selection(model, unlabelled_data, nb_data):
    ...
    ##### I add 'layers[-1]' between 'model' and 'get_output_shape_at'
    num_classes = model.layers[-1].get_output_shape_at(0)[-1]
    def get_gradient(model):
        ##### I add 'layers[0]' between 'model' and 'get_input_shape_at'
        input_shape = model.layers[0].get_input_shape_at(0)
        ##### I add 'layers[-1]' between 'model' and 'get_output_shape_at'
        output_shape = model.layers[-1].get_output_shape_at(0)
        x = K.placeholder(input_shape)
        y = K.placeholder(output_shape)
        y_pred = model.call(x)
        loss = K.mean(tf.keras.metrics.categorical_crossentropy(y, y_pred))
        weights = [tensor for tensor in model.trainable_weights]
        optimizer = model.optimizer
        gradient = optimizer.get_gradients(loss, weights)
    
        return K.function([backend.symbolic_learning_phase(), x, y], gradient)
    ...
    def compute_egl(image):    
        ...
        for k in range(num_classes):
            ...
            y_label[0,k] = 1
            grad_k = f_grad([False, image, y_label])
    ...
    n = min(500, len(unlabelled_data[0]))
    subset_index = np.random.permutation(len(unlabelled_data[0]))
    subset = unlabelled_data[0][subset_index[:n]]
...
def adversarial_selection(model, unlabelled_data, nb_data, add_adv=False, repo='.', filename = None):
    ##### I add 'layers[0]' between 'model' and 'get_input_shape_at'
    #img_size = model.layers[0].get_input_shape_at(0)
    img_size = (None, 28, 28, 1)
    img_nrows, img_ncols, n_channels = img_size[1:]
    ##### I add 'layers[-1]' between 'model' and 'get_output_shape_at'
    #nb_classes = model.layers[-1].get_output_shape_at(0)[-1]
    nb_classes = 10
...
def active_learning(num_sample, data_name, network_name, active_name,
                    nb_exp=0, nb_query=100, repo='test', filename='test.csv'):

...
##### I disable Tensorflow's eager execution to use the tf.compat.v1.placeholder() function to use a placeholder in Tensorflow 2
...
class Adversarial_example(object):
    ...
    def __init__(self, model, n_channels=3, img_nrows=32, img_ncols=32, 
        ...
        img_shape = (1, img_nrows, img_ncols, n_channels)
        ##### The placeholder function does not work with Tensorflow 2 so I change it by the tf.compat.v1.placeholder() function
        adversarial_image = tf.compat.v1.placeholder(tf.float32,shape=(1, img_nrows, img_ncols, n_channels))
        ##### The placeholder function does not work with Tensorflow 2 so I change it by the tf.compat.v1.placeholder() function
        adversarial_target = tf.compat.v1.placeholder(tf.float32,shape=(1, nb_class))
        ##### The placeholder function does not work with Tensorflow 2 so I change it by the tf.compat.v1.placeholder() function
        adv_noise = tf.compat.v1.placeholder(tf.float32,shape=(1, img_nrows, img_ncols, n_channels))
...
class Adversarial_DeepFool(Adversarial_example):
    ...
    def generate_sample_fgm(self, true_image):
        ...
        adv_image = x_i
        adv_label = self.predict(adv_image)
        ##### I reshape the adversarial image
        x_i = tf.reshape(x_i,[-1])
        ##### I reshape the image
        true_image = tf.reshape(true_image,[-1])
        ##### I replace the subtration '-' by the tf.math.subtract to subtract two tensors
        perturbation = tf.math.subtract(x_i, true_image)
        ##### I replace np.max and np.abs by tf.math.reduce_max and tf.math.abs in order to have always tensors type and not some tensors type and some numpy type
        return tf.math.reduce_max(tf.math.abs(perturbation)), x_i
    ...
    def generate_sample_bim(self, true_image):
        ...
        x_i += projected_gradient_descent(model_fn=self.model, x=x_i, eps=16, eps_iter=1e-2, nb_iter=10, norm=np.inf, loss_fn=None, clip_min=None, clip_max=None, y=true_label, targeted=False, rand_init=0, rand_minmax=None, sanity_checks=False)
        ...
        true_image = tf.reshape(true_image,[-1])
       	##### I	replace	the subtration '-' by the tf.math.subtract to subtract two tensors
        perturbation = tf.math.subtract(x_i, true_image)
        ##### I replace np.max and np.abs by tf.math.reduce_max and tf.math.abs in order to have always tensors type
        return tf.math.reduce_max(tf.math.abs(perturbation)), x_i
    ...
    def generate_sample_madry(self, true_image):
        ...
        x_i += projected_gradient_descent(model_fn=self.model, x=x_i, eps=0.3, eps_iter=1e-2, nb_iter=10, norm=np.inf, loss_fn=None, clip_min=None, clip_max=None, y=true_label, targeted=False, rand_init=None, rand_minmax=0.3, sanity_checks=False)
    ...
    def generate_sample_mim(self, true_image):
        ...
        x_i += momentum_iterative_method(model_fn=self.model, x=x_i, eps=16, eps_iter=0.06, nb_iter=10, norm=np.inf, clip_min=None, clip_max=None, y=true_label, targeted=False, decay_factor=1.0, sanity_checks=True)

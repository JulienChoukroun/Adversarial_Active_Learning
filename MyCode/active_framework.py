...
##### for the learning_phase function
...
def adversarial_selection(model, unlabelled_data, nb_data, add_adv=False, repo='.', filename = None):
    ##### I add 'layers[0]' between 'model' and 'get_input_shape_at'
    ##### For MNIST and Fashion_MNIST, the img_size is (None, 28, 28, 1) and for CIFAR-10, the img_size is (None, 32, 32, 3)
    img_size = model.layers[0].get_input_shape_at(0) 
    img_nrows, img_ncols, n_channels = img_size[1:]
    ##### I add 'layers[-1]' between 'model' and 'get_output_shape_at'
    ##### For MNIST, Fashion_MNIST and CIFAR-10, the nb_classes is 10 
...
def active_learning(num_sample, data_name, network_name, active_name,
    ...
    N_pool = 1000 ##### When we want to stop at 1000 labelled data
    print('START')
    # load data
    i=0
    while( percentage_data<=N_pool):
        ...
        ##### return # When we want to train on the all dataset
        # SAVE

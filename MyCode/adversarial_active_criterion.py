...
class Adversarial_DeepFool(Adversarial_example):
    ...
    def generate(self, data, option='fgsm'):
        ...
        for i in range(len(data)):
            r_i, x_i, adv_image = self.generate_sample(data[i:i+1], option=option)
        ...
        for i in range(len(data)):
            for j in range(len(data)):
                new_adv = sortedAdv[i]-sortedAdv[j]
                new_adv = tf.reshape(new_adv,[-1])
                adv_dist = tf.norm(new_adv, ord='euclidean')
        ...
        for i in range(1,len(data)):
            if dist[300*i+index_perturbation[i]].numpy() > median_dist:
                new_index_perturbation.append(index_perturbation[i])

        print(new_index_perturbation)
        tmp = np.array(adv_attacks)
        return new_index_perturbation, tmp[new_index_perturbation]
